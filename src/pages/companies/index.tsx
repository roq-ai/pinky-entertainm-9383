...
function CompanyListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CompanyInterface[]>(
    ...
    () =>
      getCompanies({
        relations: ['user', 'gaming.count', 'social_media.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('gaming', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>gaming</Th>}
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('gaming', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.gaming}</Td>
                    )}
                    {hasAccess('social_media', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.social_media}</Td>
                    )}
                    <Td>
