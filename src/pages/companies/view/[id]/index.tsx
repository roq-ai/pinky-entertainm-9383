...
import { deleteGamingById } from 'apiSdk/gamings';
...
function CompanyViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CompanyInterface>(
    ...
    () =>
      getCompanyById(id, {
        relations: ['user', 'gaming', 'social_media'],
      }),
  );

  const gamingHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteGamingById(id);
      await mutate();
    } catch (error) {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('gaming', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Gamings
                    </Text>
                    <NextLink passHref href={`/gamings/create?company_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.gaming?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/gamings/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.game_name}</Td>
                            <Td>{record.description}</Td>
                            <Td>
                              {hasAccess('gaming', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/gamings/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('gaming', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    gamingHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
